using System;
using System.Text;
using System.Threading;
using EnarcLabs.Phoneme.Binding;

namespace EnarcLabs.Phoneme.TestClient
{
   public class Program
    {
        public static void Main(string[] args)
        {
            //These keys should be in OpenSSL PEM format.
            //TODO: everything besides the identity command should sign message content instead of public key
            var pupKey = Convert.FromBase64String(@"");
            var privKey = Convert.FromBase64String(@"");

            /*
             * Handshake protocol description:
             * - When a new client enters the pool (calls PhonemeClient.Start()) it broadcasts a UDP packet
             *  - This packet contains the signature of the client's public key along with the key itself
             * - All peers that get this broadcast will connect to our new client and initiate a handshake over TCP
             *  - They send the "Identity" command (byte 0)
             *  - Pass their key signature and key
             *  - Optionally pass display name and profile picture
             * - Any peer may connect to another peer and send a "BinaryBlob" command (byte 1)
             *  - This has the same key signature and key pair (TODO)
             *  - It is encrypted with the public key
             */

            Console.WriteLine("Enter display name:");
            using (var client = new PhonemeClient(6969, pupKey, privKey, Console.ReadLine()))
            {
                client.Start();
                while (true)
                {
                    if (client.KnownPeers.Count >= 1)
                        break;
                    Console.WriteLine("Waiting...");
                    Thread.Sleep(1000);
                }
                Console.WriteLine("Client connected!");
                while (true)
                {
                    Console.WriteLine("Enter message:");
                    client.KnownPeers[0].SendMessage(Encoding.ASCII.GetBytes(Console.ReadLine()));
                }
            }
        }
    }
}
